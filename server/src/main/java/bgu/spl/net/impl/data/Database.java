...
public class Database {
    ...
    String userOnlyResult = executeSQL(userOnlySql);
    boolean userExists = userOnlyResult.contains("1");
    ...
    String authResult = executeSQL(authSql);

    if (!authResult.contains("1")) {
        return LoginStatus.WRONG_PASSWORD;
    }

    //  check if already logged in
    String checkLoginSql =
        "SELECT 1 FROM login_history WHERE username='" + username +
        "' AND logout_time IS NULL";
    String loginResult = executeSQL(checkLoginSql);
    ...
    if (user == null) return;
    String username = escapeSql(user.name);
    String sql =
